...
struct link_safe_ptrs
{
	...
	link_safe_ptrs(T1 &first_ptr, Args&... args){
		...
		std::shared_ptr<mutex_t> old_mtxs[] = {args.mtx_ptr ...};
...
safe_ptr<std::multimap <uint64_t, std::shared_ptr<cash_flows_t> > > safe_cash_flow_src_id;
safe_ptr<std::multimap <uint64_t, std::shared_ptr<cash_flows_t> > > safe_cash_flow_dst_id;
...
void move_money(uint64_t src_id, uint64_t dst_id, uint64_t time, int64_t money)
{
	...
	std::lock_guard<decltype(safe_cash_flow_src_id)> lock2(safe_cash_flow_src_id);
	std::lock_guard<decltype(safe_cash_flow_dst_id)> lock3(safe_cash_flow_dst_id);
	...
	safe_cash_flow_src_id->emplace(src_id, cash_flow_row_ptr);
	safe_cash_flow_dst_id->emplace(dst_id, cash_flow_row_ptr);
...
void show_total_amount()
{
	...
	for (auto it = safe_user_accounts->begin(); it != safe_user_accounts->end(); ++it){
		...
		std::cout << it->first << " => " << it->second.user_name << " , " << it->second.money << std::endl;
	}
	std::cout << "Result: all accounts total amount = " << total_amount << "\n\n";
...
void show_user_money_on_time(uint64_t user_id, uint64_t time)
{
	...
	auto in_range = safe_cash_flow_dst_id->equal_range(user_id);
	...
	in_range = safe_cash_flow_src_id->equal_range(user_id);
	for(auto it = in_range.first; it != in_range.second; ++it){
		if(it->second->time > time){
			incoming += it->second->money;
		}
	...
	std::cout << std::endl << "incoming = " << incoming << ", outcoming = " << outcoming << ", current user_money = " << user_money << std::endl;
	user_money = user_money - incoming + outcoming; // take into account cash flow
	std::cout << user_id << " => " << user_name << ", " << user_money << ", at time = " << time << " \t <<< \n\n";
